...
#check |r|
theorem my_integrable_congr{E : Type _} {F : Type _} [NormedAddCommGroup E] {f g: ℝ → E} {a : ℝ} {b : ℝ} {μ : MeasureTheory.Measure ℝ} (he : Set.EqOn f g (Set.Ico a b)) (hg : IntervalIntegrable g μ a b) : IntervalIntegrable f μ a b := by sorry

lemma Fy_measurable : ∀ (t₁ t₀  : ℝ) {htt : t₁ < t₀} { hlb : 0 ≤ t₁}, IntervalIntegrable (fun s => F (y F x₀ ε s)) volume t₁ t₀ := by
  intro t₁ t₀ htt hlb
  let a := Nat.floor (t₁/ε)
  let b := Nat.ceil (t₀/ε)
  ...
  have g_integrable : (IntervalIntegrable h volume (k*ε) ((k+1)*ε)) := by sorry
  apply my_integrable_congr _ g_integrable
